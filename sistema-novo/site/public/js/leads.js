...
function vazioLeads(){return `<div class="estado-vazio"><h2>${leads.length?'Nenhum lead encontrado':'Seu próximo atendimento começa aqui'}</h2><p>${leads.length?'Ajuste a busca ou os filtros para encontrar o contato.':'Adicione um lead ou conecte seus canais para receber os primeiros contatos.'}</p>${leads.length?'<button class="btn btn-fantasma" type="button" data-limpar-leads>Limpar filtros</button>':''}</div>`;}
function vistaLeads() {
  ...
  return `<div class="topo topo-operacional"><div><h1>Leads <span class="quantidade-titulo">${leads.length}</span></h1><div class="desc">Contatos e responsáveis de ${escapar(empresaAtual.nome)}</div></div><div class="ferramentas">${botaoNovoLead()}</div></div>
