...
begin
  -- Uma única entrega por ID é processada por vez, inclusive sob concorrência.
  if p_wa_message_id is not null then
    perform pg_advisory_xact_lock(hashtextextended(p_wa_message_id, 0));
  end if;
  ...
  if v_conversa is not null then
    update public.conversas c set
      ...
      ultima_previa = case when v_quando >= c.ultima_mensagem_em then v_previa else c.ultima_previa end,
    ...
    on conflict (empresa_id, canal, wa_id) do update set
      ...
      ultima_previa = case when excluded.ultima_mensagem_em >= c.ultima_mensagem_em then excluded.ultima_previa else c.ultima_previa end,
