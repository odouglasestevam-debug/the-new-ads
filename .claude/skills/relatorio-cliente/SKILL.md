...
description: Gera relatório de performance de mídia paga (Meta Ads) pra um cliente, num período específico, como artifact HTML com a identidade visual da The New Ads. Inclui bloco de métricas gerais, gráficos de evolução diária com marcação de fim de semana, e top criativos com miniatura em alta resolução. Use quando o usuário pedir "relatório", "relatório de performance", "relatório pro cliente X", "monta um relatório de [período]", ou pedir pra mostrar/comparar métricas de campanha num formato visual pra enviar ao cliente.
---
...
3. Estrutura recomendada: header (marca + período) → hero com 1-2 KPIs principais em destaque → métricas gerais **divididas em subgrupos temáticos pequenos** (ex: visão geral / cliques e engajamento / perfil), nunca um grid único de 15+ células → gráficos de evolução diária → top criativos → tabela de campanhas → footer com fonte.
