...
function shell(content) {
  ...
  return `<div class="layout"><aside class="sidebar"><a class="brand" href="/funil/"><img src="/funil/icone.svg" alt="" width="25" height="25">the new ads</a><a class="back-link" href="/funil/">${icon('back')} Voltar ao funil</a><div class="nav-title">Analytics</div><nav class="nav" aria-label="Áreas do analytics">${Object.entries(VIEWS).map(([key, [label]]) => `<a href="#${key}" data-nav="${key}" ${state.view === key ? 'aria-current="page"' : ''}>${icon(key)}${label}</a>`).join('')}</nav><div class="sidebar-foot"><div class="source-indicator"><i class="dot ${state.mode === 'simulado' ? 'sim' : ''}"></i>${state.mode === 'simulado' ? 'Dados simulados' : 'Dados da operação'}</div>Meta Ads + CRM próprio<br>Leitura por coorte</div></aside>
  ...
  <footer class="footer"><span>The New Ads · Analytics do funil · ${state.mode === 'simulado' ? 'Base simulada' : 'Base real'}</span><span>Atualizado às ${loadedAt.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Sao_Paulo' })} · ${go('quality', 'Método e qualidade dos dados')}</span></footer></main></div></div>`;
}
...
function render() {
  ...
  $('announcement').textContent = `${VIEWS[state.view][0]}: ${m.leads} leads. ${state.mode === 'real' ? 'Dados reais' : 'Simulação'}.`;
}
...
function exportCSV() {
  ...
  const rows = [['The New Ads', VIEWS[state.view][0]], ['Base', state.mode], ['Captação de', current.p.start, 'até', current.p.end], ['Resultados até', current.data.today], ['Nicho', state.niche || 'todos'], ['Anúncio', state.ad || 'todos'], [], ...exporting];
