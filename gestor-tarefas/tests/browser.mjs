...
try {
  ...
  assert.equal(await page.locator('.linha').count(),7);
  await page.getByRole('button',{name:'Modo eu',exact:true}).click();
  assert.equal(await page.locator('.linha').count(),4);
  ...
  await page.getByRole('button',{name:'Modo eu',exact:true}).click();
  assert.equal(await page.locator('.linha').count(),3);
  await page.evaluate(()=>{history.replaceState(null,'','#/lista/l3');renderTudo();});
  assert.equal(await page.locator('.linha').count(),1);
  assert.match(await page.locator('.linha').innerText(),/Organizar demandas/);
  ...
  await page.getByRole('button',{name:'Modo eu',exact:true}).click();
  assert.equal(await page.locator('.linha').count(),3,'OU não expande além do Modo eu');
  await page.locator('#btn-mais-filtros').click();
  await page.getByRole('button',{name:'Remover filtro Prioridade',exact:true}).click();
  assert.equal(await page.locator('.linha').count(),2);
  await page.locator('[data-multi="status"] summary').click();
